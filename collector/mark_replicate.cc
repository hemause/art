...
void MarkReplicate::BindBitmaps() {
...
MarkReplicate::MarkReplicate(Heap* heap)
    : GarbageCollector(heap, "mark and replication compaction"),
      current_space_bitmap_(nullptr), mark_bitmap_(nullptr), mark_stack_(nullptr),
      gc_barrier_(new Barrier(0)),
      mark_stack_lock_("mark sweep mark stack lock", kMarkSweepMarkStackLock),
      is_concurrent_(false), live_stack_freeze_size_(0) {
...
void MarkReplicate::InitializePhase() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
void MarkReplicate::ProcessReferences(Thread* self) {
...
void MarkReplicate::PreCleanCards() {
...
void MarkReplicate::RevokeAllThreadLocalAllocationStacks(Thread* self) {
...
void MarkReplicate::MarkingPhase() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  Thread* self = Thread::Current();
  BindBitmaps();
...
void MarkReplicate::UpdateAndMarkModUnion() {
...
void MarkReplicate::MarkReachableObjects() {
  UpdateAndMarkModUnion();
  // Recursively mark all the non-image bits set in the mark bitmap.
  RecursiveMark();
}

void MarkReplicate::ReclaimPhase() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  Thread* self = Thread::Current();
...
void MarkReplicate::FindDefaultSpaceBitmap() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  for (const auto& space : GetHeap()->GetContinuousSpaces()) {
...
void MarkReplicate::ExpandMarkStack() {
  ResizeMarkStack(mark_stack_->Capacity() * 2);
}

void MarkReplicate::ResizeMarkStack(size_t new_size) {
...
inline void MarkReplicate::MarkObjectNonNullParallel(Object* obj) {
  DCHECK(obj != nullptr);
...
mirror::Object* MarkReplicate::MarkObjectCallback(mirror::Object* obj, void* arg) {
  MarkReplicate* mark_sweep = reinterpret_cast<MarkReplicate*>(arg);
  mark_sweep->MarkObject(obj);
  return obj;
}

void MarkReplicate::MarkHeapReferenceCallback(mirror::HeapReference<mirror::Object>* ref, void* arg) {
  reinterpret_cast<MarkReplicate*>(arg)->MarkObject(ref->AsMirrorPtr());
}

bool MarkReplicate::HeapReferenceMarkedCallback(mirror::HeapReference<mirror::Object>* ref, void* arg) {
  return reinterpret_cast<MarkReplicate*>(arg)->IsMarked(ref->AsMirrorPtr());
}

class MarkReplicateMarkObjectSlowPath {
 public:
  explicit MarkReplicateMarkObjectSlowPath(MarkReplicate* mark_sweep) : mark_sweep_(mark_sweep) {
  }
 ...
 private:
  MarkReplicate* const mark_sweep_;
};

inline void MarkReplicate::MarkObjectNonNull(Object* obj) {
  DCHECK(obj != nullptr);
  ...
  if (immune_region_.ContainsObject(obj)) {
    ...
    MarkReplicateMarkObjectSlowPath visitor(this);
...
inline void MarkReplicate::PushOnMarkStack(Object* obj) {
  if (UNLIKELY(mark_stack_->Size() >= mark_stack_->Capacity())) {
...
inline bool MarkReplicate::MarkObjectParallel(const Object* obj) {
  DCHECK(obj != nullptr);
  if (kUseBakerOrBrooksReadBarrier) {
    // Verify all the objects have the correct pointer installed.
    obj->AssertReadBarrierPointer();
...
inline void MarkReplicate::MarkObject(Object* obj) {
  if (obj != nullptr) {
...
void MarkReplicate::MarkRootParallelCallback(Object** root, void* arg, const RootInfo& /*root_info*/) {
  reinterpret_cast<MarkReplicate*>(arg)->MarkObjectNonNullParallel(*root);
}

void MarkReplicate::VerifyRootMarked(Object** root, void* arg, const RootInfo& /*root_info*/) {
  CHECK(reinterpret_cast<MarkReplicate*>(arg)->IsMarked(*root));
}

void MarkReplicate::MarkRootCallback(Object** root, void* arg, const RootInfo& /*root_info*/) {
  reinterpret_cast<MarkReplicate*>(arg)->MarkObjectNonNull(*root);
}

void MarkReplicate::VerifyRootCallback(Object** root, void* arg, const RootInfo& root_info) {
  reinterpret_cast<MarkReplicate*>(arg)->VerifyRoot(*root, root_info);
}

void MarkReplicate::VerifyRoot(const Object* root, const RootInfo& root_info) {
...
void MarkReplicate::VerifyRoots() {
  Runtime::Current()->GetThreadList()->VisitRoots(VerifyRootCallback, this);
}

void MarkReplicate::MarkRoots(Thread* self) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
void MarkReplicate::MarkNonThreadRoots() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  Runtime::Current()->VisitNonThreadRoots(MarkRootCallback, this);
}

void MarkReplicate::MarkConcurrentRoots(VisitRootFlags flags) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
class ScanObjectVisitor {
 public:
  explicit ScanObjectVisitor(MarkReplicate* const mark_sweep) ALWAYS_INLINE
...
class DelayReferenceReferentVisitor {
 public:
  explicit DelayReferenceReferentVisitor(MarkReplicate* collector) : collector_(collector) {
  }
 ...
 private:
  MarkReplicate* const collector_;
};

template <bool kUseFinger = false>
class MarkStackTask : public Task {
 public:
  MarkStackTask(ThreadPool* thread_pool, MarkReplicate* mark_sweep, size_t mark_stack_size,
                Object** mark_stack)
      : mark_sweep_(mark_sweep),
  ...
  class MarkObjectParallelVisitor {
   public:
    explicit MarkObjectParallelVisitor(MarkStackTask<kUseFinger>* chunk_task,
                                       MarkReplicate* mark_sweep) ALWAYS_INLINE
  ...
  class ScanObjectParallelVisitor {
   public:
    ...
    void operator()(Object* obj) const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_)
        EXCLUSIVE_LOCKS_REQUIRED(Locks::heap_bitmap_lock_) {
      MarkReplicate* const mark_sweep = chunk_task_->mark_sweep_;
...
class CardScanTask : public MarkStackTask<false> {
 public:
  CardScanTask(ThreadPool* thread_pool, MarkReplicate* mark_sweep,
...
size_t MarkReplicate::GetThreadCount(bool paused) const {
...
void MarkReplicate::ScanGrayObjects(bool paused, byte minimum_age) {
...
class RecursiveMarkTask : public MarkStackTask<false> {
 public:
  RecursiveMarkTask(ThreadPool* thread_pool, MarkReplicate* mark_sweep,
...
void MarkReplicate::RecursiveMark() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
mirror::Object* MarkReplicate::IsMarkedCallback(mirror::Object* object, void* arg) {
  if (reinterpret_cast<MarkReplicate*>(arg)->IsMarked(object)) {
    return object;
  }
  return nullptr;
}

void MarkReplicate::RecursiveMarkDirtyObjects(bool paused, byte minimum_age) {
  ScanGrayObjects(paused, minimum_age);
  ProcessMarkStack(paused);
}

void MarkReplicate::ReMarkRoots() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
void MarkReplicate::SweepSystemWeaks(Thread* self) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  WriterMutexLock mu(self, *Locks::heap_bitmap_lock_);
  Runtime::Current()->SweepSystemWeaks(IsMarkedCallback, this);
}

mirror::Object* MarkReplicate::VerifySystemWeakIsLiveCallback(Object* obj, void* arg) {
  reinterpret_cast<MarkReplicate*>(arg)->VerifyIsLive(obj);
  // We don't actually want to sweep the object, so lets return "marked"
  return obj;
}

void MarkReplicate::VerifyIsLive(const Object* obj) {
...
void MarkReplicate::VerifySystemWeaks() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
class CheckpointMarkThreadRoots : public Closure {
 public:
  explicit CheckpointMarkThreadRoots(MarkReplicate* mark_sweep,
  ...
  virtual void Run(Thread* thread) OVERRIDE NO_THREAD_SAFETY_ANALYSIS {
    ...
    thread->VisitRoots(MarkReplicate::MarkRootParallelCallback, mark_sweep_);
    ATRACE_END();
...
void MarkReplicate::MarkRootsCheckpoint(Thread* self,
                                    bool revoke_ros_alloc_thread_local_buffers_at_checkpoint) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
void MarkReplicate::SweepArray(accounting::ObjectStack* allocations, bool swap_bitmaps) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  Thread* self = Thread::Current();
...
void MarkReplicate::Sweep(bool swap_bitmaps) {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
...
void MarkReplicate::SweepLargeObjects(bool swap_bitmaps) {
...
void MarkReplicate::DelayReferenceReferent(mirror::Class* klass, mirror::Reference* ref) {
...
class MarkObjectVisitor {
 public:
  explicit MarkObjectVisitor(MarkReplicate* const mark_sweep) ALWAYS_INLINE : mark_sweep_(mark_sweep) {
  }
...
void MarkReplicate::ScanObject(Object* obj) {
...
void MarkReplicate::ProcessMarkStackCallback(void* arg) {
  reinterpret_cast<MarkReplicate*>(arg)->ProcessMarkStack(false);
}

void MarkReplicate::ProcessMarkStackParallel(size_t thread_count) {
  Thread* self = Thread::Current();
  ThreadPool* thread_pool = GetHeap()->GetThreadPool();
...
void MarkReplicate::ProcessMarkStack(bool paused) {
...
inline bool MarkReplicate::IsMarked(const Object* object) const {
...
void MarkReplicate::FinishPhase() {
  TimingLogger::ScopedTiming t(__FUNCTION__, GetTimings());
  if (kCountScannedTypes) {
    VLOG(gc) << "MarkReplicate scanned classes=" << class_count_.LoadRelaxed()
...
void MarkReplicate::RevokeAllThreadLocalBuffers() {
